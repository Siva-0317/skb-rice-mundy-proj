...
import { getCategories, getItems, addItem, updateItem, setItemActive, seedIfEmpty } from '../firebase/items';
import { Pencil, ChevronDown, ChevronRight, Plus, X } from 'lucide-react';
...
export default function ItemsList() {
  ...
  const [formData, setFormData] = useState({
    name: '', categoryKey: '', bagKg: '', rate: '', mrp: '', stock: ''
  });
  ...
  const handleOpenModal = (item = null) => {
    if (item) {
      setEditingItem(item);
      setFormData({
        name: item.name, categoryKey: item.categoryKey, bagKg: item.bagKg,
        rate: item.rate, mrp: item.mrp, stock: item.stock
      });
    } else {
      setEditingItem(null);
      setFormData({
        name: '', categoryKey: categories[0]?.key || '', bagKg: '',
        rate: '', mrp: '', stock: ''
      });
    }
    setIsModalOpen(true);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        name: formData.name, categoryKey: formData.categoryKey, bagKg: Number(formData.bagKg),
        rate: Number(formData.rate), mrp: Number(formData.mrp),
      ...
      if (editingItem) {
        await updateItem(editingItem.id, {
          categoryKey: payload.categoryKey, bagKg: payload.bagKg,
          rate: payload.rate, mrp: payload.mrp
        });
      } else {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden border border-border flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center p-5 border-b border-border">
              ...
              <button onClick={handleCloseModal} className="text-textMuted hover:text-textDark transition-colors p-2"><X className="w-5 h-5" /></button>
            </div>
            <form onSubmit={handleSubmit} className="p-5 overflow-y-auto">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2">
                  ...
                  <input type="text" required disabled={!!editingItem} value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full px-3 py-2 rounded-lg border border-border text-base sm:text-sm focus:outline-none focus:ring-2 focus:ring-gold/50 disabled:bg-panel disabled:text-textMuted min-h-[44px]" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-textDark mb-1">Category</label>
                  <select required value={formData.categoryKey} onChange={(e) => setFormData({...formData, categoryKey: e.target.value})} className="w-full px-3 py-2 rounded-lg border border-border text-base sm:text-sm focus:outline-none focus:ring-2 focus:ring-gold/50 bg-white min-h-[44px]">
                    {categories.map(cat => <option key={cat.key} value={cat.key}>{cat.label}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-textDark mb-1">Bag Size (kg)</label>
                  <input type="number" required min="1" step="0.1" value={formData.bagKg} onChange={(e) => setFormData({...formData, bagKg: e.target.value})} className="w-full px-3 py-2 rounded-lg border border-border text-base sm:text-sm focus:outline-none focus:ring-2 focus:ring-gold/50 min-h-[44px]" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-textDark mb-1">Rate (₹)</label>
                  <input type="number" required min="0" step="0.01" value={formData.rate} disabled={!isOwner} onChange={(e) => setFormData({...formData, rate: e.target.value})} className="w-full px-3 py-2 rounded-lg border border-border text-base sm:text-sm focus:outline-none focus:ring-2 focus:ring-gold/50 disabled:bg-panel disabled:text-textMuted min-h-[44px]" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-textDark mb-1">MRP (₹)</label>
                  <input type="number" required min="0" step="0.01" value={formData.mrp} disabled={!isOwner} onChange={(e) => setFormData({...formData, mrp: e.target.value})} className="w-full px-3 py-2 rounded-lg border border-border text-base sm:text-sm focus:outline-none focus:ring-2 focus:ring-gold/50 disabled:bg-panel disabled:text-textMuted min-h-[44px]" />
                </div>
                <div className="sm:col-span-2">
                  <label className="block text-sm font-medium text-textDark mb-1">Opening Stock (bags)</label>
                  <input type="number" required={!editingItem} disabled={!!editingItem} min="0" value={formData.stock} onChange={(e) => setFormData({...formData, stock: e.target.value})} className="w-full px-3 py-2 rounded-lg border border-border text-base sm:text-sm focus:outline-none focus:ring-2 focus:ring-gold/50 disabled:bg-panel disabled:text-textMuted min-h-[44px]" />
                </div>
              </div>
              <div className="flex justify-end gap-3 mt-8">
                <button type="button" onClick={handleCloseModal} className="px-4 py-2 rounded-lg font-medium text-sm text-brownDark border border-brownDark hover:bg-brownDark/5 transition-colors min-h-[44px]">Cancel</button>
